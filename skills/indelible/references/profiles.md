...
- **Marking:** after `ind key open`, copy the learner's files to a new folder in the system temp folder (outside their project and the workspace), add the hidden tests, run them with the learner's own toolchain and a time limit, then read the code. Record a verdict per question, then delete that temporary folder. The learner's files are never changed. The first time, say once that their build tool may download the dependencies the project declares. A time limit that works everywhere (macOS has no `timeout`; on Windows use `py -3`), run in the copy's folder:
  ```
